...
export function MapPreview({ className, dense = false }: { className?: string; dense?: boolean }) {
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-2xl border border-slate-200 bg-[linear-gradient(135deg,#eef2ff_0%,#e0e7ff_35%,#dbeafe_100%)]",
        className
      )}
    >
      ...
      {pins.map((p, i) => (
        <span
          key={i}
          className={cn("absolute -translate-x-1/2 -translate-y-full drop-shadow-sm", p.color)}
